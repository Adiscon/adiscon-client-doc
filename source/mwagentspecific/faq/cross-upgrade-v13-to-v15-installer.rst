.. _mwagent-cross-upgrade-v13-to-v15-installer:

Why did my MonitorWare Agent update report success when the files were not upgraded correctly?
==============================================================================================

Question
--------

Why can an in-place MonitorWare Agent update report success while the
...
This behavior has been confirmed for direct MonitorWare Agent updates from
the 13.x generation to the 15.x generation. Other direct source versions can
show the same symptoms. The source version alone does not prove that an update
path is safe or affected.
...
* the MonitorWare Agent About dialog or file properties still show an older
...
1. Export or back up the MonitorWare Agent configuration before changing the
   installation.
2. Close the MonitorWare Agent Configuration Client and stop the MonitorWare
   Agent service manually before each setup run. Stopping the service first
   can avoid a reboot request caused by locked service files, but it cannot
   avoid every reboot request.
3. If a staged update is practical, use a tested intermediate release, such
   as 13.x to 14.x to 15.x, and verify the client, service, features, and
...
4. If the installation remains inconsistent, use the clean-install procedure
   for MonitorWare Agent after exporting the configuration.
